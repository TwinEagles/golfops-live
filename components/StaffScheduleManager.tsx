...
type Staff={id:string;employee_name:string;job_title:string;start_time:string|null;end_time:string|null;duty:string|null;zone:string|null;status:string;notes:string|null};
...
export default function StaffScheduleManager({date,staff,tasks,isAdmin,latestImport}:{date:string;staff:Staff[];tasks:Task[];isAdmin:boolean;latestImport:any}){
 ...
 async function post(body:any){const r=await fetch('/api/staff-tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||'Unable to save.');return d;}
 async function add(e:FormEvent){e.preventDefault();setError('');try{await post({action:'add',title,area,dueDate:due,description});setTitle('');setDescription('');router.refresh();}catch(e){setError(e instanceof Error?e.message:'Unable to save.');}}
 async function status(id:string,status:string){try{await post({action:'status',id,status});router.refresh();}catch(e){setError(e instanceof Error?e.message:'Unable to save.');}}
 const grouped=areas.map(([key,name])=>[name,staff.filter(s=>s.status==='SCHEDULED'&&(s.duty||'').toUpperCase().includes(name.toUpperCase().split(' ')[0]))]).filter(([,rows])=>(rows as Staff[]).length) as [string,Staff[]][];
